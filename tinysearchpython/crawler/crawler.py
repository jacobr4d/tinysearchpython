...
class Crawler:
    ...
    def crawl(self):
        with open(hitsPath, "w") as hitsFile, open(linksPath, "w") as linksFile:
            while (True):
                ...
                for _, _, link, _ in html.iterlinks():
                    try:
                        new_url = Url(link)
                        print(url, new_url, file=linksFile)
                ...
                # filter is word [a-zA-Z]
                # regularize case (lowercase)
                # filter is not stop word     NOT DOING
                # apply stemmer               NOT DOING
